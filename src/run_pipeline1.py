...
def run_pipeline1(
    ...
    for group_start in range(0, len(remaining_pis), group_size):
        ...
        if dry_run:
            ...
            result = write_pipeline1_results(
                group_members, config, tab_name=tab_name, deactivate_missing=False
            )
    ...
    # ------------------------------------------------------------------
    # Final deactivation pass (scope: institutions processed this run)
    # ------------------------------------------------------------------
    if not dry_run and all_members_written:
        scope_insts = {pi["institution"] for pi in all_pis}
        logger.info(
            "Final deactivation pass for institutions: %s", scope_insts
        )
